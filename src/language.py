...
_LANGUAGE_INTENT_URL = EINSTEIN_VISION_URL+"/v2/language/intent"
_LANGUAGE_ENTITIES_URL = EINSTEIN_VISION_URL+"/v2/language/entities"
...
def predict_for_intent(token, model_id, document):
    ...
    return _prediction_request(_LANGUAGE_INTENT_URL, payload, headers)


def predict_for_entities(token, model_id, document):
    """Run a prediction against a model using image url.
    ...
    payload = {'document': (None, document),
               ...
               'modelId': (None, model_id)}
    headers = {'Authorization': 'Bearer ' + token, 'Cache-Control': 'no-cache'}
    return _prediction_request(_LANGUAGE_ENTITIES_URL, payload, headers)
